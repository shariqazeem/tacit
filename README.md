...
Tacit ships an **MCP server** ([`mcp/`](mcp/)) so any MCP-capable agent (Claude Code / Desktop) can run real Canton commerce as **its own party** and get real contract ids back. It's a thin client of the app's HTTP API. **Primary (Tacit Work):** `tacit_work_health` · `tacit_assess_vendor` `{ url, maxBudget, policyId? }` · `tacit_probe_performance` `{ url, maxBudget, policyId? }` · `tacit_procure_work` `{ url, maxBudget, jobId?, buyerLabel? }` — an agent procures real work (no fallback; errors if the network isn't ready). **Market intelligence:** `tacit_market_overview {}` — the agent economy from the auditor's lawful view (totals, per-provider treasuries + win-shares, recent receipt commitments); sealed bids and report bodies never appear, so an agent can check a provider's track record before hiring without seeing anything private. **Negotiation demo:** `tacit_procure` `{ description, maxBudget, buyerName? }` · `tacit_my_deals` `{ buyerName }` · `tacit_health` · `tacit_explain_privacy`. Agent-to-agent isolation is real: `tacit_my_deals` for one agent never returns another agent's deals — the ledger, not the app, decides who sees what.
